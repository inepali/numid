...
export default function AdminPage() {
  ...
  if (isAdmin === false) {
    return (
      <div className="min-h-screen bg-black flex flex-col justify-center items-center px-6 text-center text-slate-400">
        <ShieldAlert className="w-16 h-16 text-red-500/80 mb-6 animate-pulse" />
        <h2 className="font-display text-2xl font-bold text-white mb-2">Access Forbidden</h2>
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-display font-extrabold text-white text-lg tracking-tight">
              Num<span className="text-indigo-400">ID</span> <span className="text-slate-500 text-xs font-semibold ml-1 px-1.5 py-0.5 rounded bg-slate-900 border border-white/5 uppercase tracking-wider">Admin</span>
            </span>
          </div>

          <Link
            href="/dashboard"
            className="text-xs bg-slate-900 hover:bg-slate-800 text-slate-300 border border-white/10 font-bold px-3.5 py-2 rounded-xl transition-colors flex items-center gap-1.5"
          >
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 space-y-8">
        ...
        {errorMsg && (
          <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-300 text-xs flex items-start space-x-2.5">
            ...
            <div>
              <p className="font-bold text-red-200">Admin Command Error</p>
        ...
        {successMsg && (
          <div className="p-4 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 text-xs flex items-start space-x-2.5">
            ...
            <div>
              <p className="font-bold text-indigo-200">Admin Command Success</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="p-5 rounded-2xl bg-slate-950 border border-white/5 relative shadow-md">
            ...
            <p className="text-2xl font-extrabold text-white mt-1 font-mono">{stats?.totalUsers}</p>
          </div>
          ...
          <div className="p-5 rounded-2xl bg-slate-950 border border-white/5 relative shadow-md">
            ...
            <span className="text-[10px] uppercase font-bold text-slate-500 tracking-wider">Active Forwards</span>
            <p className="text-2xl font-extrabold text-white mt-1 font-mono">{stats?.activeUsers}</p>
          </div>
          ...
          <div className="p-5 rounded-2xl bg-slate-950 border border-white/5 relative shadow-md">
            ...
            <span className="text-[10px] uppercase font-bold text-slate-500 tracking-wider">Pending Setups</span>
            <p className="text-2xl font-extrabold text-white mt-1 font-mono">{stats?.pendingUsers}</p>
          </div>

          <div className="p-5 rounded-2xl bg-slate-950 border border-white/5 relative shadow-md">
            <div className="w-8 h-8 rounded-lg bg-pink-500/10 flex items-center justify-center text-pink-400 mb-3">
              <Smartphone className="w-4.5 h-4.5" />
            </div>
            <span className="text-[10px] uppercase font-bold text-slate-500 tracking-wider">SMS OTP Success</span>
            <div className="flex items-center space-x-2 mt-1">
              <span className="text-2xl font-extrabold text-white font-mono">{stats?.smsSuccessRate}%</span>
              <span className="text-[10px] text-slate-500 font-mono">({stats?.smsVerified}/{stats?.smsSent})</span>
            </div>
          </div>
        ...
        <div className="p-8 rounded-3xl bg-slate-950 border border-white/5 shadow-xl space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h3 className="font-display font-bold text-white text-lg">User Directory</h3>
              <p className="text-xs text-slate-500">Query and manage forwarding channels, verify triggers, or disable routes.</p>
            </div>
            ...
            <form onSubmit={handleSearch} className="relative w-full sm:w-[320px]">
              <input
                ...
                className="w-full bg-slate-900 border border-white/5 focus:border-indigo-500/40 rounded-xl py-2.5 pl-10 pr-4 text-xs text-white focus:outline-none"
              />
              <button type="submit" className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500 hover:text-white transition-colors">
          ...
          {/* User records list */}
          <div className="overflow-x-auto w-full border border-white/5 rounded-2xl bg-slate-900/10 backdrop-blur">
                ...
                {users.length === 0 ? (
                  ...
                  users.map((u) => (
                    <tr key={u.id} className="hover:bg-slate-900/20 transition-colors">
                      <td className="p-4">
                        <p className="font-bold text-white">{u.phone_number}</p>
                        <p className="text-[10px] text-indigo-400 font-mono mt-0.5">{u.numid_address}</p>
                      </td>
                      <td className="p-4">
                        <p className="text-slate-300 font-medium">{u.destination_email}</p>
                        <p className="text-[9px] text-slate-500 mt-0.5">Joined: {new Date(u.created_at).toLocaleDateString()}</p>
                      </td>
                      <td className="p-4 space-y-1.5">
                        ...
                        <div className="flex items-center space-x-1.5">
                          ...
                          <span className="text-[10px] text-slate-400">Email Verification</span>
                        </div>
                      </td>
                      <td className="p-4 font-mono text-slate-500 text-[10px]">
                        {u.cloudflare_route_id ? (
                          <span className="text-slate-400 select-all">{u.cloudflare_route_id}</span>
                        ) : (
                          <span className="italic text-slate-600">None</span>
                        )}
                      </td>
                      <td className="p-4">
                        <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold font-mono border ${u.status === "active" ? "bg-emerald-500/20 text-emerald-300 border-emerald-500/30" : u.status === "disabled" ? "bg-red-500/20 text-red-300 border-red-500/30" : "bg-orange-500/20 text-orange-300 border-orange-500/30"}`}>
                      ...
                      <td className="p-4 text-right space-x-2">
                        ...
                        <button
                          ...
                          disabled={isPending}
                          className={`px-2.5 py-1.5 rounded-lg font-bold text-[10px] border transition-colors ${u.status === "disabled" ? "bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border-emerald-500/20" : "bg-orange-500/10 hover:bg-orange-500/20 text-orange-400 border-orange-500/20"}`}
                        ...
                        <button
                          ...
                          className="px-2.5 py-1.5 bg-slate-900 hover:bg-slate-800 text-slate-300 border border-white/10 font-bold text-[10px] rounded-lg transition-colors inline-flex items-center gap-1 disabled:opacity-40 disabled:cursor-not-allowed"
                        ...
                        <button
                          ...
                          disabled={isPending}
                          className="px-2.5 py-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 font-bold text-[10px] rounded-lg transition-colors inline-flex items-center"
        ...
        <div className="p-8 rounded-3xl bg-slate-950 border border-white/5 shadow-xl">
          <div className="flex items-center space-x-2.5 mb-6">
            ...
            <h3 className="font-display font-bold text-white text-lg">System-Wide Audit Trails</h3>
          </div>
              ...
              {systemLogs.map((log) => (
                <div key={log.id} className="relative text-left">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between text-xs text-slate-400">
                    <div className="flex items-center space-x-2">
                      <span className="font-bold text-slate-300 uppercase tracking-wider">{log.action.replace(/_/g, " ")}</span>
                      <span className="text-[10px] text-slate-600 font-mono">({log.id.substring(0, 8)})</span>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-6">
          <div className="w-full max-w-md bg-slate-950 border border-red-500/20 rounded-3xl p-6 shadow-2xl animate-scaleIn">
            <div className="flex items-center space-x-2 text-red-400 mb-2">
              ...
              <h3 className="font-display font-bold text-white text-lg">Force Account Teardown?</h3>
            </div>
            <p className="text-xs text-slate-400 mb-6 leading-relaxed">
              This will permanently delete user record <strong className="text-white font-mono">{deleteTargetId}</strong> from our database, cancel their Cloudflare forwarding rule, and drop their Supabase authentication row.
            </p>
